...
from rl.environments.carla.navigation.agent import Agent
...
from rl.environments.carla.navigation.global_route_planner_dao import GlobalRoutePlannerDAO
from rl.environments.carla.navigation.types_behavior import Cautious, Aggressive, Normal

from rl.environments.carla.tools.misc import get_speed, positive
...
class BehaviorAgent(Agent):
    ...
    def update_information(self):
        ...
        if self.direction is None:
            self.direction = RoadOption.LANE_FOLLOW
        ...
        if self.incoming_direction is None:
            self.incoming_direction = RoadOption.LANE_FOLLOW
    ...
    def collision_and_car_avoid_manager(self, location, waypoint):
        ...
        if self.direction == RoadOption.CHANGE_LANE_LEFT:
            vehicle_state, vehicle, distance = self._bh_is_vehicle_hazard(
                waypoint, location, vehicle_list, max(
                    self.behavior.min_proximity_threshold, self.speed_limit / 2), up_angle_th=180, lane_offset=-1)
        elif self.direction == RoadOption.CHANGE_LANE_RIGHT:
            vehicle_state, vehicle, distance = self._bh_is_vehicle_hazard(
                waypoint, location, vehicle_list, max(
                    self.behavior.min_proximity_threshold, self.speed_limit / 2), up_angle_th=180, lane_offset=1)
        else:
            ...
            if vehicle_state and self.direction == RoadOption.LANE_FOLLOW and \
            ...
            elif not vehicle_state and self.direction == RoadOption.LANE_FOLLOW \
    ...
    def run_step(self, debug=False):
        ...
        elif (self.incoming_waypoint is not None) and self.incoming_waypoint.is_junction and \
