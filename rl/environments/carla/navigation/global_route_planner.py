...
class GlobalRoutePlanner(object):
            ...
            if road_id in self._road_id_to_edge and section_id in self._road_id_to_edge[road_id] and lane_id in self._road_id_to_edge[road_id][section_id]:
                ...
                if path:
                    ...
                    self._graph.add_edge(
                        ...
                        intersection=end_wp.is_junction, type=RoadOption.LANE_FOLLOW)
                ...
                if not segment['entry'].is_junction:
                    ...
                    if waypoint.right_lane_marking.lane_change & carla.LaneChange.Right and not right_found:
                        ...
                        if next_waypoint is not None and next_waypoint.lane_type == carla.LaneType.Driving and waypoint.road_id == next_waypoint.road_id:
                            next_road_option = RoadOption.CHANGE_LANE_RIGHT
                    ...
                    if waypoint.left_lane_marking.lane_change & carla.LaneChange.Left and not left_found:
                        ...
                        if next_waypoint is not None and next_waypoint.lane_type == carla.LaneType.Driving and waypoint.road_id == next_waypoint.road_id:
                            next_road_option = RoadOption.CHANGE_LANE_LEFT
                            next_segment = self._localize(next_waypoint.transform.location)
                            if next_segment is not None:
                                self._graph.add_edge(
                                    self._id_map[segment['entryxyz']], next_segment[0], entry_waypoint=waypoint,
    ...
    def _successive_last_intersection_edge(self, index, route):
        ...
        for node1, node2 in [(route[i], route[i+1]) for i in range(index, len(route)-1)]:
            ...
            if candidate_edge['type'] == RoadOption.LANE_FOLLOW and candidate_edge['intersection']:
                last_intersection_edge = candidate_edge
                last_node = node2
            else:
                break
    ...
    def _turn_decision(self, index, route, threshold=math.radians(35)):
        ...
        if index > 0:
            if self._previous_decision != RoadOption.VOID and self._intersection_end_node > 0 and self._intersection_end_node != previous_node and next_edge['type'] == RoadOption.LANE_FOLLOW and next_edge['intersection']:
                decision = self._previous_decision
            else:
                self._intersection_end_node = -1
                current_edge = self._graph.edges[previous_node, current_node]
                calculate_turn = current_edge['type'] == RoadOption.LANE_FOLLOW and not current_edge[
                    'intersection'] and next_edge['type'] == RoadOption.LANE_FOLLOW and next_edge['intersection']
                if calculate_turn:
                    ...
                    for neighbor in self._graph.successors(current_node):
                        ...
                        if select_edge['type'] == RoadOption.LANE_FOLLOW:
    ...
    def trace_route(self, origin, destination):
        ...
        for i in range(len(route) - 1):
            ...
            if edge['type'] != RoadOption.LANE_FOLLOW and edge['type'] != RoadOption.VOID:
