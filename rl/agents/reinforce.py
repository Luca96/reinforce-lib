...
class ReinforceAgent:
    ...
    def __init__(self, learning_rates=(3e-4, 3e-4), optimization_steps=(1, 2), name='reinforce-agent',
                 gamma=0.99, lambda_=0.95, target_kl=0.01, seed=None, weights_dir='weights'):
        self.memory = None
    ...
    def predict(self, state):
        # TODO: remove value prediction
        action = self.policy_network(state, training=False)
        log_prob = action.log_prob(action)
        value = self.value_network(state, training=False)
        # log_prob = self.distribution.log_prob(value=action)
        # print(action[0].numpy(), log_prob[0].numpy())
    ...
    def update(self, batch_size: int):
        ...
        for step, (states_batch, advantages_batch) in enumerate(policy_batches):
            with tf.GradientTape() as tape:
                ...
                # self.stats['policy_losses'].append(policy_loss.numpy())
        ...
        for step, (states_batch, returns_batch) in enumerate(value_batches):
            with tf.GradientTape() as tape:
                ...
                # self.stats['value_losses'].append(value_loss.numpy())
    ...
    def policy_gradient_objective(self, states, advantages):
        # distribution = self.policy_network()
        # actions = self.policy_network.predict_on_batch(states)
        actions = self.policy_network(states, training=True)

        # return -tf.reduce_mean(actions.log_prob(actions) * advantages)
        return -actions.log_prob(actions) * advantages

    def learn(self, environment, episodes: int, timesteps: int, subsampling_fraction=0.25, load=False, save=True):
